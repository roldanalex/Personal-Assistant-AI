...
- **Safe Web Lookup Tool**: Lucy can perform controlled web lookups via a registered `perform_google_search` tool using Google Custom Search (requires `google_search_api_key` and `google_search_engine_id`).
- **Quick Test Script**: Run `Rscript scripts/check_google_search.R` from the project root to verify Google CSE credentials and connectivity before using web lookups.
...
- User (end-user): "Search the web for recent studies on active learning in high school math and summarize the top findings."
- Developer (programmatic): `perform_google_search(query = "active learning high school math", n = 3)`
